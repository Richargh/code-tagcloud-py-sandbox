...
def behavior_cloud() -> list:
    return [Token.Name.Function, Token.Name.Attribute]


def draw_java_tag_cloud(folder: str, token_types: list, show: bool) -> None:
    _draw_code_tag_cloud(
        folder,
        '/**/*.java',
        partial(extract_code_tags, JavaLexer(), token_types),
        show)

def draw_kotlin_tag_cloud(folder: str, token_types: list, show: bool) -> None:
    _draw_code_tag_cloud(
        folder,
        '/**/*.kt',
        partial(extract_code_tags, KotlinLexer(), token_types),
        show)

def draw_csharp_tag_cloud(folder: str, token_types: list, show: bool) -> None:
    _draw_code_tag_cloud(
        folder,
        '/**/*.cs',
        partial(extract_code_tags, CSharpLexer(), token_types),
        show)

def draw_ts_tag_cloud(folder: str, token_types: list, show: bool) -> None:
    _draw_code_tag_cloud(
        folder,
        '/**/*.ts',
        partial(extract_code_tags, TypeScriptLexer(), token_types),
        show)

def draw_js_tag_cloud(folder: str, token_types: list, show: bool) -> None:
    _draw_code_tag_cloud(
        folder,
        '/**/*.js',
        partial(extract_code_tags, TypeScriptLexer(), token_types),
